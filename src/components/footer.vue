...
<template>
    ...
    <div v-if="this.$store.state.themeObj.user_start!=0" class="footBack footBackQian">
        <!-- <div class="footer-img" :style="{background:this.$store.state.themeObj.bottom_img?'url('+this.$store.state.themeObj.bottom_img+') no-repeat 50%':'url(../img/footer01.png) no-repeat 50%'}">
        </div> -->
        <div class="fcontainer">
            ...
            <p>
                <span>Made with</span> <img class="fheart" src="../../public/img/heart02.png"/>  <span>by Marin.</span>
            </p>
        </div>
    </div>
...
<script>
    export default {
        data() { //选项 / 数据
            return {
                footBg:"url(../img/footer01.png) no-repeat 50%",//底部图片
                longTime:'',
            }
        },
        methods: { //事件处理器
            runTime:function(){//运行倒计时
                var that = this;
                var oldTime =new Date('2021/9/19 00:00:00');
