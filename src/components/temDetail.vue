			...
			<el-row :class="pdonate?'donate-body':'donate-body donate-body-show'" :gutter="30">
				<el-col :span="12" class="donate-item">
					<div class="donate-tip">
						<img :src="detailObj.wechat_image?detailObj.wechat_image: '../img/tou.jpg'" :onerror="$store.state.errorImg" />
						<span>微信扫一扫，向我赞赏</span>
					</div>
				</el-col>
				<el-col :span="12" class="donate-item">
					<div class="donate-tip">
						<img :src="detailObj.alipay_image?detailObj.alipay_image:'../img/tou.jpg'" :onerror="$store.state.errorImg" />
...
export default {
	...
	methods: { //事件处理器
		...
		likecollectHandle: function (islike) {//用户点击喜欢1,用户点击收藏2
			var that = this
		...
		routeChange: function () {
			...
			getArticleInfo(that.aid, that.userid, function (msg) {
				that.detailObj = msg
				that.likecount = msg.likecount ? msg.likecount : 0
				that.collectcount = msg.collectcount ? msg.collectcount : 0
				that.likeArt = msg.userlikestart == 0 ? false : true
				that.collectArt = msg.usercollectstart == 0 ? false : true
