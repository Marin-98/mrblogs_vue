...
<div class="">
				...
				<div class="headBox">
					<el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" @select="handleSelect" :router="true">
						...
						<el-submenu index="/Share">
							...
							<el-menu-item v-for="(item,index) in classListObj" :key="'class1'+index" :index="'/Share?classId='+item.classId">{{item.name}}</el-menu-item>
						</el-submenu>
						<!-- <el-submenu index="/Aboutme">
							<template slot="title"><i class="fa fa-wa fa-flask"></i> 技术日记</template>
						...
						<el-menu-item index="/Friendslink"><i class="fa fa-wa fa-users"></i> 友情链接</el-menu-item>
	...
	<div class="headImgBox" :style="{backgroundImage:this.$store.state.themeObj.top_image?'url('+this.$store.state.themeObj.top_image+')':'url(../img/05.png)'}">
		<div class="scene">
			<div><span id="luke"></span></div>
		</div>
		<div class="h-information">
			<a href="#/Aboutme">
                    <img :src="this.$store.state.themeObj.head_portrait?this.$store.state.themeObj.head_portrait:'../img/tou.png'" alt="">
                </a>
			<h2 class="h-description">
                    <a href="#/Aboutme">
                        {{this.$store.state.themeObj.autograph?this.$store.state.themeObj.autograph:"止言上智，止心上律"}}
                    </a>
                </h2>
		</div>
	</div>
...
export default {
	data() { //选项 / 数据
		return {
			...
			// path:'',//当前打开页面的路径
			input: '', //input输入内容
			headBg: 'url(../img/headbg05.jpg)', //头部背景图
			headTou: '', //头像
			projectList: '' //项目列表
		}
	},
	...
	methods: { //事件处理器
		...
		routeChange: function() {
			...
			// navMenList(function(msg) { //技术日记项目列表获取
			// 	// console.log('技术日记',msg);
