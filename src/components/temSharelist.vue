	...
	<el-row class="sharelistBox">
		<div v-if="this.$route.name=='Share'&&!this.$route.query.keywords" class="shareTitle">
			...
			<ul v-if="sonclassList" class="shareclassTwo">
				<li v-for="(citem,index) in sonclassList" :key="'class'+index">
					<a :href="'#/Share?classId='+classId+'&classtwoId='+citem.classid" :class="citem.classid==classtwoId?'active':''">{{citem.catename}}</a>
				</li>
			</ul>
		</div>
...
import { ShowArticleAll, initDate,getCateList } from '../utils/server.js'
...
export default {
	...
	methods: { //事件处理器
		...
		showSearchShowList: function (initpage) {//展示数据
			...
			if(that.$route.query.classId!=null){
				getCateList(function(msg) { //文章分类
				        that.shareClass = msg
						console.log(that.shareClass)
						that.className=that.shareClass[that.$route.query.classId-1].name
		        })
			}
			// that.className=shareClass[1].name
