...
// import base, {encode} from 'js-base64'
//公共路径
let portUrl = "http://" + window.location.hostname + ":8888/";
...
const getArticleInfo = (artId, userid, callback) => {
	let url = portUrl + 'article/getArticleInfo?art_id=' + artId + '&userid=' + userid;
	axios.get(url).then(num => {
		if (num.data.code == 1001) {
			callback && callback(num.data.data);
		} else {
...
const ArticleComment = (artId, commentId, callback) => {
	let url = portUrl + 'comment/ArticleComment?art_id=' + artId + '&commentid=' + commentId;
	axios.get(url).then(num => {
		callback && callback(num.data);
	})
}
...
const OtherComment = (leaveId, commentId, callback) => { //分类类型ID（1：赞赏 2：友情链接 3：留言板 4：关于我）
	let url = portUrl + 'comment/OtherComment?leaveid=' + leaveId + '&commentid=' + commentId;
	axios.get(url).then(num => {
		callback && callback(num.data);
	})
}
...
//文章评论
const setArticleComment = (content, userid, article_id, leavepid, pid, callback) => {
	let url = portUrl + 'comment/setArticleComment?content=' + content + '&userid=' + userid + '&article_id=' + article_id + '&leavepid=' + leavepid + '&pid=' + pid;
	axios.get(url).then(num => {
		callback && callback(num.data);
	})
}
...
//其他评论
const setOuthComment = (content, userid, article_id, leaveid, leavepid, pid, callback) => {
	let url = portUrl + 'comment/setOuthComment?content=' + content + '&userid=' + userid + '&article_id=' + article_id + '&leaveid=' + leaveid + '&leavepid=' + leavepid + '&pid=' + pid;
	axios.get(url).then(num => {
		callback && callback(num.data);
	})
}
...
const getArtLikeCollect = (userid, artId, islike, callback) => {
	var url = '';
	if (islike == 1) {
		url = portUrl + 'article/getArtLike?userid=' + userid + '&art_id=' + artId;
	} else {
		url = portUrl + 'article/getArtCollect?userid=' + userid + '&art_id=' + artId;
	}
	axios.get(url).then(num => {
		if (num.data.code == 1001) {
			callback && callback(num.data.msg);
...
const getLikeCollectList = (userid, artId, articleName, islike, callback) => {
	var url = '';
	if (islike == 1) {
		url = portUrl + 'article/getLikeList?userid=' + userid + '&art_id=' + artId + '&article_name=' + articleName;
	} else {
		url = portUrl + 'article/getCollectList?userid=' + userid + '&art_id=' + artId + '&article_name=' + articleName;
	}
	axios.get(url).then(num => {
		callback && callback(num.data);
	})
...
const getUserInfo = (userid, callback) => {
	let url = portUrl + 'Userinfo/getUserInfo?userid=' + userid;
	axios.get(url).then(num => {
		if (num.data.code == 1001) {
			callback && callback(num.data);
		} else {
...
const UserInfoSave = (obj, callback) => {
	...
	var data = {
		...
		'image': obj.image,
		'label': obj.label,
		'headStart': obj.head_start,
		'logoStart': obj.logo_start
	};
	// console.log(data);
	axios.get(url, {
		params: data
...
const publishArt = (title, image, description, content, classId, catename, callback) => {
	// content = encode(content);
	let Base64 = require('js-base64').Base64;
	content = Base64.encode(content);
	console.log(content);
	let url = portUrl + 'article/Publish?title=' + title + '&image=' + image + '&description=' + description + '&content=' + content + '&classId=' + classId + "&catename=" + catename;
	// console.log(url);
	axios.get(url).then(num => {
		console.log(num);
		callback && callback(num.data);
...
const getCateList = (callback) => {
	let url = portUrl + 'cate/catelist';
	axios.get(url).then(num => {
		if (num.data.code == 1001) {
			callback && callback(num.data.data);
		} else if (num.data.code == 1005) {
